...
cat /tmp/Flat.txt| while read line; do
...
printf "\n" >>/tmp/OUTPUT
counter=$((counter+1))
echo $counter "of "$Records" Events Processed"
done
...
cat /tmp/OUTPUT |sed 's/<[^>]\+>/ /g'|sed 's/^ *//; s/ *$//; /^$/d'|sed 's/ \+/,/g'>Security.evtx.csv
echo "File Security.evtx.csv created!"
echo ""
echo ""
cat Security.evtx.csv|grep ",NTLM," |grep -vc ANONYMOUSLOGON |awk '{printf $0 " Matches found in lateral movement/Pass the Hash search:\n"}'|tee PTH-lateral.csv
