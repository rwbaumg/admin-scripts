...
if ! RAW_OUTPUT=$(grep -vP '^(ERR|MIS)' /proc/interrupts | tail -n+2 | awk '{printf "%s ", $1; for(j=(NF-2);j<=NF;j++){if($j~/^[A-Za-z]/){printf "%s ", $j} }; if($j=NF){printf "\n";} }' && grep -P '^(ERR|MIS)' /proc/interrupts | awk '{ printf "%s %s\n", $1, $2}'); then
